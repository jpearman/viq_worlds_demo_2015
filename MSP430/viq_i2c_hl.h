...
/**
 * These registers are used by code to communicate sensor specific information
 * to the VEX IQ brain.  Extra space for this project
 * @name Extended user registers                                               */
/*-----------------------------------------------------------------------------*/
///@{
#if defined (__MSP430G2553__) || defined(__DOXYGEN__)
#define USERE_REG_MIN            0xA0    ///< Low extent of user register region
/// High extent of extended user register region
#define USERE_REG_MAX            0xBF    // 32 registers
#endif
///@}
...
#if defined (__MSP430G2553__) || defined(__DOXYGEN__)
    /// storage for additional sensor specific registers
    volatile    uint8_t userE[ USERE_REG_MAX - USERE_REG_MIN + 1 ];
#endif
} i2cDataStruct;
...
uint8_t *viq_i2c_get_extended_data_ptr(void);

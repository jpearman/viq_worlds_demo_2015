...
#endif
{
    ...
    if (UCB0CTL1 & UCTR)
        ...
        // Read extended user data
        if( i2cData->reg_address >= USERE_REG_MIN && i2cData->reg_address <= USERE_REG_MAX )
            UCB0TXBUF = i2cData->userE[(uint16_t)(i2cData->reg_address - USERE_REG_MIN)];
#endif
        else
            UCB0TXBUF = 0;
    ...
    // Master must be sending us a byte
        ...
        if(i2cData->state == kI2cStateAddress)
            ...
            // Write to extended user data
            if( i2cData->reg_address >= USERE_REG_MIN && i2cData->reg_address <= USERE_REG_MAX )
                i2cData->userE[(uint16_t)(i2cData->reg_address - USERE_REG_MIN)] = UCB0RXBUF;
#endif
            else

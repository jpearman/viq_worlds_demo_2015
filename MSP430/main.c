...
temp_task(void)
{
    ...
    static   int16_t  temp_filtered = 28*4; // starting point
...
int main(void) {
    ...
    // First task is effectively highest priority
    // in as much as it will have more stable timing
    // run matrix update 500 times per second
    viq_task_add( viq_matrix_task,    2,"LED1",  0 );
    ...
    // check for new message every 40mS
    viq_task_add( viq_matrix_update, 40,"LED2",  0 );

    // Add the temperature reading task
    // run five times per second
    viq_task_add( temp_task,  200,"TEMP",       0 );

...
/*-----------------------------------------------------------------------------*/
/** @brief     Get a pointer to the extended user registers                    */
/** @returns   Pointer to the extended user registers                          */
/*-----------------------------------------------------------------------------*/

uint8_t *
viq_i2c_get_extended_data_ptr()
{
#if defined (__MSP430G2553__) || defined(__DOXYGEN__)
    return( (uint8_t *)i2cData.userE );
#else
    return NULL;
#endif
}
